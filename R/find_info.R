...
find_info <- function(x, reqtime = 0) {
    ...
    if(length(projectlist) == 0) {
        ...
        data.frame(id = x, name = NA, projects = NA, project_id = NA, anzahl_projekte = NA,
                   affiliation = NA, stringsAsFactors = F)
    } else {
        data.frame(id, name, projects, project_id, anzahl_projekte, affiliation,
                   stringsAsFactors = F)
    }
